...
def broadcast_target_kb() -> InlineKeyboardMarkup:
    ...
    b.button(text="😴 Challenjdda yo'q (premium)", callback_data="bc_target:inactive")
    b.button(text="📍 Viloyat bo'yicha",           callback_data="bc_target:region")
    b.adjust(1)
    return b.as_markup()
...
def never_paid_kb(uid: int) -> InlineKeyboardMarkup:
    ...
    b.button(text="✅ Qo'lda premium",    callback_data=f"usr_give_prem:{uid}")
    b.adjust(2)
    return b.as_markup()
...
def user_detail_kb(uid: int, is_premium: bool) -> InlineKeyboardMarkup:
    ...
    b.button(text="← Ro\'yxatga",         callback_data="admin:users")
    b.adjust(1)
    return b.as_markup()
...
def challenge_info_kb() -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    b.button(text="💳 To'lovga o'tish →", callback_data="go_payment")
    return b.as_markup()
...
def payment_method_kb() -> InlineKeyboardMarkup:
    ...
    b.button(text="💳 Karta (o'tkazma)", callback_data="pay_method:card")
    b.adjust(1)
    return b.as_markup()
...
def after_payment_kb() -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    b.button(text="✅ To'lov qildim, chek yuboraman", callback_data="send_receipt")
    return b.as_markup()
...
# ════ CHALLENGE ════
def challenge_kb(completed: list[int], current: int = 1) -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    for day in range(1, 31):
        if day in completed:     text = f"✅ {day}"
        elif day == current:     text = f"🔥 {day}"
        else:                    text = str(day)
...
def day_detail_kb(day: int) -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    b.button(text="🍽️ Ovqatlar",       callback_data=f"meals:{day}")
...
def meals_kb(day: int, meals: list, logged: list[int]) -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
...
def meal_detail_kb(day: int, idx: int, logged: bool) -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    b.button(text="↩️ Bekor" if logged else "✅ Yedim!",
             callback_data=f"meal_toggle:{day}:{idx}")
    b.button(text="← Ovqatlar", callback_data=f"meals:{day}")
    b.adjust(1)
    return b.as_markup()

def exercises_kb(day: int, exs: list, logged: list[int]) -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
...
def admin_kb() -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    b.button(text="✅ Kutayotgan to'lovlar",    callback_data="admin:payments")
